...
class SummaryRequest(models.Model):
    ...
    def create(self, vals):
        ...
        res = super(SummaryRequest, self).create(vals)
        for line in res.leave_line:
            start_date, end_date = line.manipulate_options(line.date, line.this_day_hour_id.id,
                                                           line.next_day_hour_id.id, line.distinct_shift, line.first,
                                                           line.second)
            start_date = start_date.replace(tzinfo=None)#datetime.strftime(start_date, '%Y-%m-%d %H:%M:%S')
            end_date = end_date.replace(tzinfo=None)#datetime.strftime(end_date, '%Y-%m-%d %H:%M:%S')
            if line.start_date != start_date or line.end_date != end_date:
