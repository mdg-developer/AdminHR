...
class Binary(http.Controller):
    ...
    def download_function_descriptions(self, employee=None, employee_name=None):
        ...
        if employee:            
            ...
            # import pdb
            # pdb.set_trace() 
            attachment =request.env['ir.attachment'].sudo().search([('res_model','=','contract.config'),('name','=','Employment Contract')], order="id desc", limit=1)
            ...
            contract_doc = open('/tmp/EC_WB_HO.docx', 'rb').read();open('/tmp/EC_WB_HO.docx', 'wb').write(contract_doc)
            return request.make_response(contract_doc,[('Content-Type', 'application/msword'),('Content-Disposition', 'attachment')])

    @http.route('/web/binary/download_docx_template_report/<int:employee>/<string:employee_name>/<string:doc_name>', type='http', auth="public")
    def download_doc_template_function_descriptions(self, employee=None, employee_name=None,doc_name=None):
        ...
        if employee:
            ...
            doc_template_id = request.env['document.template.config'].sudo().search([('name', '=', doc_name)], order="id desc", limit=1)
            contract_file_path = attachment._full_path(attachment.store_fname)
            # request.env['contract.config'].search([('employee','=',employee)]).contract_file
            # contract_file_path = get_module_resource('employment_contract', 'data', 'EC_WB_HO.docx')
            f = open(contract_file_path, 'rb')
                    ...
                    for i in range(len(inline)):
                        ...
                        for j in range(len(words)):
                            ...
                            if doc_template_id and not get_word:
                                emp = request.env['hr.employee'].sudo().search([('id', '=', employee)], order="id desc", limit=1)
                                for line_word in doc_template_id.template_lines:
                                    if word == 'today':
                                        ...
                                        if tmp_id:
                                            val_id = tmp_id[line_word.model_field.name][1] #self.env[line_word.model_name.model].sudo().search([('id', '=', tmp_id)])
                                            word = val_id
                                            get_word = True
                                        break
