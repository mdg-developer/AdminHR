...
class HrLeaveAutoAllocation(models.Model):
    ...
    def compute_leave_balance_probation_employee(self, employee_id, holiday_status_id,leave_allocation,joining_date,fiscal_year):
        if not leave_allocation.carried_forward:
            return True
        ...
        upcoming_fiscal_year = self.env['account.fiscal.year'].sudo().search(
            [('date_from', '<=', today_plus_one), ('date_to', '>=', today_plus_one)], limit=1)
        if employee_id and holiday_status_id:
            ...
            leaves_taken = 0
            today = fields.Date.today()
            prev_fiscal_year = self.env['account.fiscal.year'].sudo().search(
                [('date_from', '<=', date_from)], order='date_from desc',limit=1)
            start_date = prev_fiscal_year.date_from
            end_date = prev_fiscal_year.date_to
            requests = self.env['hr.leave'].sudo().search([
                ('employee_id', '=', employee_id),
            ...
            number_of_days = (max_leaves - leaves_taken) * 0.5
            if max_leaves == 0 and number_of_days == 0 and not allocations:
                number_of_days = leave_allocation.maximum_allocation_days
                emp = self.env['hr.employee'].search([('id','=',employee_id)])
                if emp.joining_date and emp.joining_date.year == fiscal_year.date_from.year:
                    number_of_days = (number_of_days / 12) * ((date_to.month + 1) - joining_date.month)
                if number_of_days > 0:
            ...
            if number_of_days > 0:
                ...
                if number_of_days > 0:#if not existing_allocation and number_of_days > 0:
                    values = {
