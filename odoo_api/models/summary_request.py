...
class LeaveSummaryRequest(models.Model):
        ...
        if start_date and end_date and employee:
            ...
            for single_date in (start_date + timedelta(n) for n in range(day_count)):
                ...
                for new_line_value in new_line_values:
                    ...
                    new_line_value.update({'request_date': date, 'date': date, 'start_date': local_start_date, 'end_date': local_end_date})
                    distinct_shift = new_line_value['distinct_shift']
                    next_day_hour_id = new_line_value['next_day_hour_id']
                    new_line_value.update(leave_line_obj._compute_allow_edit(single_date, start_date, end_date, next_day_hour_id, distinct_shift))
    ...
    def get_employee_calendar_date(self,employee_id=None,start_date=None, end_date=None):
        ...
        trip_bill_arry = []
        calendar_arry = []
        for travel in travel_obj.search([('employee_id','=',employee_id),('start_date','>=',start_date),('end_date','<=',end_date),('state','=','approve')]):
        ...
        for leave in leave_obj.search([('employee_id','=',employee_id),('date_from','>=',start_date +' 00:00:00'),('date_to','<=',end_date +' 23:59:59'),('state','=','validate')]):
        ...
        for training in training_obj.search([('employee_id','=',employee_id),('company_id','=',emp_id.company_id.id),('end_date','<=',end_date)]):
        ...
        domain = [('to_datetime','<=',end_date +' 23:59:59'),('state','=','approve'),'|',('spare1_id','=',employee_id),('spare2_id','=',employee_id)]
        #emp_id = self.env['hr.employee'].browse([employee_id])
        if emp_id.address_id:
            domain = [('from_datetime','>=',start_date +' 00:00:00'),('to_datetime','<=',end_date +' 23:59:59'),('state','=','approve'),'|',('spare1_id','=',employee_id),('spare2_id','=',employee_id),('driver_id','=',emp_id.address_id.id)]
        ...
        domain = [('to_datetime','<=',end_date +' 23:59:59'),('state','=','approve'),('spare_id','=',employee_id)]
        emp_id = self.env['hr.employee'].browse([employee_id])
        if emp_id.address_id:
            ...
            domain = [('to_datetime','<=',end_date +' 23:59:59'),('state','=','approve'),'|',('spare_id','=',employee_id),('driver_id','=',emp_id.address_id.id)]
        ...
        for calendar_id in calendar_obj.search([('name','ilike',emp_id.name),('start','>=',start_date +' 00:00:00'),('stop','<=',end_date +' 23:59:59')]):
    ...
    def create_leave_summary_request(self, employee_id=None, holiday_status_id=None, start_date=None, end_date=None, duration=None, description=None, attachment=None, file_name=None, leave_line=None):
        ...
        if employee_id:
            ...
            if leave_line and leave:
                for line in leave_line:
                    start_date = datetime. strptime(line.get('start_date'), '%Y-%m-%d %H:%M:%S') - relativedelta(hours=6, minutes=30)
                    end_date = datetime. strptime(line.get('end_date'), '%Y-%m-%d %H:%M:%S') - relativedelta(hours=6, minutes=30)
    ...
    def get_employee_calendar_date_new(self,employee_id=None,start_date=None, end_date=None):
        ...
        for leave in leave_obj.search([('employee_id','=',employee_id),('date_from','>=',str(start_date) +' 00:00:00'),('date_to','<=',str(end_date) +' 23:59:59'),('state','=','validate')]):
            leave_array.append({'employee_id': leave.employee_id.id,
                           'start_date': leave.date_from.strftime(DATE_FORMAT),
                           'end_date': leave.date_to.strftime(DATE_FORMAT),
                           'name': leave.holiday_status_id.name,
        ...
        domain = [('from_datetime','>=',str(start_date) +' 00:00:00'),('to_datetime','<=',str(end_date) +' 23:59:59'),('state','=','open'),('driver_id','=',employee_id)]
        ...
        for calendar_id in calendar_obj.search([('name','ilike',emp_id.name),('start','>=',str(start_date) +' 00:00:00'),('stop','<=',str(end_date) +' 23:59:59')]):
