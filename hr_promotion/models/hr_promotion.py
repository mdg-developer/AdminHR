...
class EmployeePromotion(models.Model):
    ...
    def onchange_employee_id(self):
        if self.employee_id:
            ...
            if employee.contract_ids:
                ...
                self.job_grade_id = contract.job_grade_id.id
                self.salary_level_id = contract.salary_level_id.id
                self.struct_id = contract.struct_id.id
                if self.company_id == self.new_company_id:
                    self.new_struct_id = contract.struct_id.id
                self.wage = contract.wage
                self.new_job_grade_id = contract.job_grade_id.id
                self.new_salary_level_id = contract.salary_level_id.id
    ...
    def create(self, vals):
        ...
        if vals['company_id'] != vals['new_company_id'] or vals['branch_id'] != vals['new_branch_id'] or vals['department_id'] != vals['new_department_id'] or vals['job_id'] != vals['new_job_id']:
    ...
    def button_approve(self, employee_id=None):
        ...
        if resume_line:
            resume_line.write({'date_end':self.date - timedelta(days=1),
                               'description': self.company_id.name + ' / ' + self.branch_id.name + ' / ' + self.department_id.complete_name
                               })
            line_id = self.env['hr.resume.line'].sudo().create(
                {'name': self.new_job_id.name,
                 ...
                 'description': self.new_company_id.name + ' / ' + self.new_branch_id.name + ' / ' + self.new_department_id.complete_name
                 }
            )
        if self.company_id != self.new_company_id:
            ...
            for loan in self.env['hr.loan'].sudo().search([('employee_id','=',self.employee_id.id),('company_id','=',self.company_id.id),
                                        ('branch_id','=',self.branch_id.id),('balance_amount','>',0),('state','=','approve')]):
        ...
        if contract:
            contract.write({'state': 'close'})#, 'date_end': self.date})
    ...
    def constrain_new_job(self):
        for rec in self:
            ...
            if rec.new_job_id.id:
                job_line = self.env['job.line'].sudo().search([('company_id', '=', rec.new_company_id.id),
                                                                ('branch_id', '=', rec.new_branch_id.id),
                                                                ('department_id', '=', rec.new_department_id.id),
                                                                ('job_id', '=', rec.new_job_id.id)])
                if not job_line:
                    raise ValidationError(_('%s position at %s, %s, %s is not expecting new employee.') % (rec.new_job_id.name, rec.new_department_id.complete_name, rec.new_branch_id.name,
                                                                                                           rec.new_company_id.name))

                if job_line and job_line.new_employee <= 0:
                    raise ValidationError(_('%s position at %s, %s, %s is not expecting new employee.') % (rec.new_job_id.name, rec.new_department_id.complete_name, rec.new_branch_id.name, rec.new_company_id.name))