...
class HrAnnouncements(models.Model):
    ...
    def _announcement_count(self):
        ...
        for rec in self:
            ...
            ann_ids_dep = self.env['hr.announcement'].sudo().search([('department_ids', 'in', [rec.department_id.id]),
                                                                     ...
                                                                     ('date_start', '<=', now_date),
                                                                     ('date_end', '>=', now_date)])
            ann_ids_job = self.env['hr.announcement'].sudo().search([('job_grade_ids', 'in', [rec.job_grade_id.id]),
    ...
    def announcement_view(self):
        ...
        for each in ann_obj:
            ann_ids.append(each)
        view_id = self.env.ref('hr_announcement.view_hr_announcement_form').id
        if ann_ids:
            if len(ann_ids) > 1:
                value = {
