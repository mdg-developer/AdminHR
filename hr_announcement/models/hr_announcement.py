...
class HrAnnouncement(models.Model):
    ...
    def announcement_schedule(self):
        announcement_ids = self.env['hr.announcement'].search([('date_start','=',datetime.date(datetime.today())), ('state','=','approved')])
        for announce_id in announcement_ids:
            domain = [('user_id', '=', announce_id.create_uid.id)]
            for emp_id in announce_id.env['hr.employee'].sudo().search(domain):
                ...
                announce_id.env['one.signal.notification.message'].create(one_signal_values)
