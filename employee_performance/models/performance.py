...
#             if today_date >= rec.end_from_date and today_date <= rec.end_to_date:
    ...
    def action_update_state(self):
        ...
        return {
            'name': _('Change State Employee Performance'),
            'view_mode': 'form',
            'res_model': 'employee.performance.wizard',
            'view_id': self.env.ref('employee_performance.employee_performance_wizard_form').id,
...
class KeyPerformanceAttachment(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Key Performance Attachment'
