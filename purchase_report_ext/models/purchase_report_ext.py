...
class Maintenance(models.Model):    
    ...
    qty_balanced = fields.Float('Qty Balanced', readonly=True)
        ...
        select_str = """
            WITH currency_rate as (%s)
                SELECT
                    ...
                    (sum(l.product_qty / line_uom.factor * product_uom.factor) - l.qty_received) as qty_balanced,
                    (sum(l.product_qty / line_uom.factor * product_uom.factor) - l.qty_received) * (sum(l.product_qty * l.price_unit / COALESCE(po.currency_rate, 1.0))/NULLIF(sum(l.product_qty/line_uom.factor*product_uom.factor),0.0))::decimal(16,2) as amount,
