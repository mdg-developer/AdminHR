   ...
   FROM ( SELECT row_number() OVER (ORDER BY a.level) AS id,
           ...
           FROM ( SELECT rc.id,
                ...
                ( SELECT DISTINCT he.id,
                   ...
                   FROM hr_employee he,
                    hr_job hj
                  WHERE he.job_id = hj.id AND he.active = true AND he.id not in(select manager_id from hr_department where manager_id is not null and active = 't') AND NOT (he.id IN ( SELECT res_branch.manager_id
                           FROM res_branch
                ...
                ( SELECT DISTINCT he.id,
                   ...
                   FROM hr_employee he,
                    hr_job hj
                  WHERE he.job_id = hj.id AND he.active = true AND he.id not in(select manager_id from hr_department where manager_id is not null and active = 't') AND parent_id is not null
                  ORDER BY he.name)
                UNION ALL
                (
