...
class HrLeaveAutoAllocation(models.Model):
    ...
    def run_existing_employee_leave_allocation(self,employee_ids,fiscal_year,upcoming_fiscal_year):
        ...
        if employee_ids:
            ...
            domain = [('state', '=', 'validate'), ('employee_id', 'in', employees.ids), ('carry_leave', '!=', True),
                      ('fiscal_year', '=', fiscal_year.id)]
        else:
        ...
        for auto in auto_allocations:
            ...
            for emp in employees:
                ...
                if not allocation_obj.search(
                        [('employee_id', '=', emp.id), ('holiday_status_id', '=', holiday_status.id),
                         ('carry_leave', '!=', True), ('state', '=', 'validate'),
                         ('fiscal_year', '=', upcoming_fiscal_year.id)]):
                    if holiday_status.allocation_type != 'fixed':
                        ...
                        if auto.prorated and auto.prorated_based_on == 'join date':
                            ...
                            if auto.validity_year_type == 'fiscal':
                                if (date_from < emp.joining_date and emp.joining_date < date_to):
                                    number_of_days = (number_of_days / 12) * (
                                                (date_to.month + 1) - emp.joining_date.month)
                                    # print('hello')
                                    self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days,
                                                           fiscal_year)
                                else:
                                    self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days,
                                                           fiscal_year)
                                ...
                                if (
                                        start_date_current_year < emp.joining_date and emp.joining_date < end_date_current_year):
                                    number_of_days = (number_of_days / 12) * (13 - emp.joining_date.month)
                                    self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days,
                                                           fiscal_year)
                                else:
                                    self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days,
                            ...
                            if auto.validity_year_type == 'fiscal':
                                if (date_from < emp.permanent_date and emp.permanent_date < date_to):
                                    number_of_days = (number_of_days / 12) * (
                                                (date_to.month + 1) - emp.permanent_date.month)
                                    # print('hello')
                                    self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days,
                                                           fiscal_year)
                                else:
                                ...
                                if (
                                        start_date_current_year < emp.permanent_date and emp.permanent_date < end_date_current_year):
        ...
        if fiscal_year and today == fiscal_year.date_to:
                    ...
                    if not allocation_obj.search([('employee_id', '=', emp.id), ('holiday_status_id', '=', holiday_status.id), ('carry_leave', '!=', True), ('state', '=', 'validate'), ('fiscal_year', '=', upcoming_fiscal_year.id)]):
                        if holiday_status.allocation_type != 'fixed':
                            ...
                            if auto.prorated and auto.prorated_based_on == 'join date':
                                ...
                                if auto.validity_year_type == 'fiscal':
                                    if (date_from<emp.joining_date and emp.joining_date<date_to):
                                        number_of_days = (number_of_days/12)*((date_to.month+1)-emp.joining_date.month)
                                        # print('hello')
                                        self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                    else:
                                        self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                else:
                                    if (start_date_current_year<emp.joining_date and emp.joining_date<end_date_current_year):
                                        number_of_days = (number_of_days/12)*(13-emp.joining_date.month)
                                        self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                    else:
                                        self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                    ...
                                    if auto.validity_year_type == 'fiscal':
                                        if (date_from<emp.permanent_date and emp.permanent_date<date_to):
                                            number_of_days = (number_of_days/12)*((date_to.month+1)-emp.permanent_date.month)
                                            # print('hello')
                                            self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                        else:
                                            self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                    else:
                                        if (start_date_current_year<emp.permanent_date and emp.permanent_date<end_date_current_year):
                                            number_of_days = (number_of_days/12)*(13-emp.permanent_date.month)
                                            self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                                        else:
                                            self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
            ...
            for auto in auto_allocations:
                ...
                for emp in employees:
                    ...
                    #if emp.joining_date and emp.joining_date.year == fiscal_year.date_from.year:
                    self.compute_leave_balance_probation_employee(emp.id,holiday_status.id,auto,emp.joining_date,fiscal_year)
                    self.run_existing_employee_leave_allocation([emp.id],fiscal_year,upcoming_fiscal_year)
                    if not allocation_obj.search([('employee_id', '=', emp.id), ('holiday_status_id', '=', holiday_status.id), ('carry_leave', '!=', True), ('state', '=', 'validate')]):
                        if holiday_status.allocation_type != 'fixed':
                            ...
                            if emp.state == 'permanent':
                                ...
                                if auto.prorated and auto.prorated_based_on == 'join date':
                                        ...
                                        if auto.validity_year_type == 'fiscal':
                                            if (date_from<emp.joining_date and emp.joining_date<date_to):
                                                totalmonth= (13-emp.joining_date.month + date_to.month) if emp.joining_date.month > 9 else ((date_to.month+1)-emp.joining_date.month)

                                                number_of_days = (number_of_days/12)* totalmonth #((date_to.month+1)-emp.joining_date.month)
                                                if number_of_days % 0.5 != 0:
                                                    number_of_days = round(number_of_days)
                                                # print('hello')
                                                self.create_allocation(holiday_status, emp, date_from, date_to, number_of_days, fiscal_year)
                        ...
                        if existing_allocation:
                            number_of_days = self.get_number_of_days(auto, emp)
                            if emp.joining_date and emp.joining_date.year == fiscal_year.date_from.year:
                                if number_of_days > 0 and emp.joining_date.day > 15:
