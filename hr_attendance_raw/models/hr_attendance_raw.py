...
#                 raise ValidationError(_("Company Name '%s' is not found") % self.company)
    ...
    @api.constrains('fingerprint_id', 'company', 'attendance_datetime')
    ...
    def import_attendances(self):
        ...
        for raw in self:
            ...
            if raw.id in [7681258,7681257,7696665,7696318,8824624,8824613]:#[7523703,7523704,7523705,7523710,7523711]:
                print("hello")
        #for raw in self.browse([1409918,1409919]):
        #for raw in self.browse([24456,24457,24459,24460]):
                        ...
                        if distinct_shift and night_shift:
                            if distinct_shift.check_out:
                                ...
                                if distinct_shift.check_in > raw.attendance_datetime:
                                    if distinct_shift.check_in > night_shift.check_in:
                                        ...
                                        distinct_shift.write({'check_in': raw.attendance_datetime, 'check_out': check_out_time,'missed':False})
                            ...
                            if distinct_shift.check_out:
                                ...
                                if distinct_shift.check_in > raw.attendance_datetime:
                                    ...
                                    distinct_shift.write({'check_in': raw.attendance_datetime, 'check_out': distinct_check_out,'missed':False})
                                    att_value.update({'employee_id': employee.id, 'check_in': night_check_in, 'check_out': day_stop,'missed':False})
                                elif  distinct_shift.check_in < raw.attendance_datetime <= distinct_date_and_time:
                                    distinct_shift.write({'check_out': raw.attendance_datetime,'missed':False})
                                    ...
                                    if 16 <= att_date.hour <= 18:
                                        att_value.update({'employee_id': employee.id, 'check_in': raw.attendance_datetime,'check_out': day_stop,'missed':False})
                                    else:
                    ...
                    try:
                        if morning_shift and night_shift:
                            if distinct_hour:
                                if night_shift.check_out:
                                    ...
                                    if night_shift.check_in < raw.attendance_datetime:
                                        ...
                                        night_shift.write({'check_in': raw.attendance_datetime, 'check_out': night_check_out,'missed':False})
                                    else:
                                        night_check_in = morning_shift.check_out
                                        night_check_out = night_shift.check_in
                                        morning_shift.check_out = raw.attendance_datetime
                                        night_shift.write({'check_in': night_check_in, 'check_out': night_check_out,'missed':False})
                            else:
                            ...
                            if not distinct_hour:
                                att_value.update({'check_out': day_stop,'missed':False})

                        elif night_shift:
                            if distinct_hour:
                                if night_shift.check_out:
                                    if raw.attendance_datetime < night_shift.check_in:
                                        att_value.update({'employee_id': employee.id, 'check_in': day_start, 'check_out': raw.attendance_datetime,'missed':False})
                                    elif night_shift.check_in < raw.attendance_datetime < night_shift.check_out:
                                        att_value.update({'employee_id': employee.id, 'check_in': day_start, 'check_out': night_shift.check_in,'missed':False})
                                        night_shift.check_in = raw.attendance_datetime
                                    else:
                                        att_value.update({'employee_id': employee.id, 'check_in': day_start, 'check_out': night_shift.check_in,'missed':False})
                                        night_check_in = night_shift.check_out
                                        night_shift.write({'check_in': night_check_in, 'check_out': raw.attendance_datetime,'missed':False})
                                else:
                                    if night_shift.check_in < raw.attendance_datetime:
                                        night_shift.check_out = raw.attendance_datetime
                                    else:
...
#                 attendance = False
#                 if attendance_id:
#                     if attendance_id.check_in.date == raw.attendance_datetime.date:
#                         attendance = False
#                     else:
                ...
                if attendance :
                        ...
                        if attendance.check_out:
                            if attendance.check_in > raw.attendance_datetime:
                                ...
                                if attendance.check_out.date() == raw.attendance_datetime.date():                                    
                                    update_value.update({'check_out': raw.attendance_datetime,'missed':False})
                                else:
                            ...
                            if attendance.employee_id.resource_calendar_id.holiday == True and attendance.employee_id.resource_calendar_id.hours_per_day < 9:
                                future_date_and_time = attendance.check_in
                            else:
                                future_date_and_time = attendance.check_in + timedelta(hours=2)

                            if attendance.check_in > raw.attendance_datetime:
                                check_out = attendance.check_in
                                update_value.update({'check_in': raw.attendance_datetime, 'check_out': check_out,'missed':False})
                    ...
                    try:
                        ...
                        if morning_hour and att_date.hour > 1 and att_date.hour <= 7:
                            hr_att_id = attendance_obj.create({'employee_id': employee.id, 'check_in': day_start,'check_out':raw.attendance_datetime,'missed':False})
                            
                        elif night_hour:
                            hr_att_id = attendance_obj.create({'employee_id': employee.id, 'check_in': raw.attendance_datetime,'check_out':day_stop,'missed':False})
