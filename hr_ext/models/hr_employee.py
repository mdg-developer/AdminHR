...
class Employee(models.Model):
    ...
    approve_manager = fields.Many2one('hr.employee', string='Approve Manager', check_company=False,
    ...
    address_home_id = fields.Many2one(
        ...
        groups="hr.group_hr_user,account.group_account_invoice,account.group_account_manager,account.group_account_user,fleet.fleet_group_user,purchase.group_purchase_user",
        tracking=True,
    ...
    def toggle_active(self):
        ...
        if self.active:
            latest_contract = self.env['hr.contract'].sudo().search(
                [('employee_id', '=', self.id), '|', ('active', '=', True), ('active', '=', False)], order='id desc',
                limit=1)
    ...
    def one_signal_reminder(self, force_approve=False):
        ...
        if passport_expiry_date_add:
            ...
            for emp in employees:
                one_signal_values = {'employee_id': emp.id,
                                     ...
                                     'contents': _('Passport Expiry Date for %s -%s') % (
                                     emp.name, emp.passport_expiry_date),
        ...
        if driver_license_expiry_date_add:
            ...
            for emp in employees:
                one_signal_values = {'employee_id': emp.id,
                                     'message_type': 'reminder',
                                     'contents': _('Driving License Expiry Date for %s -%s') % (
                                     emp.name, emp.dl_expired_date),
            ...
            for fleet in self.env['fleet.vehicle'].search(domain):
                if fleet.hr_driver_id:
                    one_signal_values = {'employee_id': fleet.hr_driver_id.id,
                                         'message_type': 'reminder',
                                         'contents': _('Fleet License Expiry Date for %s -%s') % (
                                         fleet.hr_driver_id.name, fleet.license_expired_date),
        ...
        if probation_expiry_date_add:
            ...
            for emp in employees:
                ...
                if emp.branch_id.hr_manager_id:
                    one_signal_values = {'employee_id': emp.branch_id.hr_manager_id.id,
                                         'message_type': 'reminder',
                                         'contents': _('Probation Expire Date for %s -%s') % (
                                         emp.name, emp.trial_date_end),
                                         'headings': _('WB B2B : Probation Expire')}
                    one_signal_id = self.env['one.signal.notification.message'].create(one_signal_values)
        ...
        if maintenance_expiry_date_add:
            ...
            domain = [('start_date', '>=', maintenance_expiry_date + ' 00:00:00'),
                      ('start_date', '<=', maintenance_expiry_date + ' 23:59:59'), ('state', '=', 'approved')]
            employees = self.env['maintenance.request'].search(domain)
            for emp in employees:
                if emp.driver_id:
                    one_signal_values = {'employee_id': emp.driver_id.id or False,
                                         'message_type': 'reminder',
                                         'contents': _('Maintenance Expire Date for %s -%s') % (
                                         emp.driver_id.name, emp.start_date),
    ...
    def write(self, vals):
        ...
        if vals.get('dotted_line_manager_id'):
            ...
            if dotted_line_manager_id:
                self.message_post(
                    body="Dotted Line Manager " + old_dotted_line_manager_id + "->" + dotted_line_manager_id.name)
        if vals.get('job_id'):
