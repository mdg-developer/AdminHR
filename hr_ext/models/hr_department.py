    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            job_emp_ids = employee_obj.sudo().search(
                [('company_id', '=', company_id), ('branch_id', '=', branch_id),('department_id','=',department_id), ('job_id', '=', old_job_id)])
                    ...
                    if employee.id != old_manager_id:
                        if vals.get('manager_id'):
                            ...
                            if employee.id != vals.get('manager_id'):
                                employee.write({'manager_job_id': emp.job_id.id,'parent_id':vals.get('manager_id'), 'approve_manager': res.approve_manager.id})
                            else:
                                ...
                                if job_line and job_line.upper_position:
                                    ...
                                    if direct_mng:
                                        employee.write(
                                            {'manager_job_id': job_line.upper_position.id, 'parent_id': direct_mng.id, 'approve_manager': approve_manager_id.id})
                        else:
                            job_line = job_line_obj.sudo().search(
                                [('job_id', '=', res.job_id.id), ('company_id', '=', company_id),
                                 ('branch_id', '=', branch_id), ('department_id', '=', res.id)], limit=1)
                        ...
                        job_line = job_line_obj.sudo().search([('job_id','=',res.job_id.id),('company_id','=',company_id),('branch_id','=',branch_id),('department_id','=',res.id)],limit=1)
                        if job_line and job_line.upper_position:
                            direct_mng = employee_obj.sudo().search([('company_id','=',job_line.company_id.id),
                                                            ('branch_id','=',job_line.branch_id.id),('job_id','=',job_line.upper_position.id)],limit=1)
                            if direct_mng:
                                employee.write({'manager_job_id':job_line.upper_position.id,'parent_id':direct_mng.id, 'approve_manager': approve_manager_id.id})
                    ...
                    if employee.id != approve_manager_id:
                        if vals.get('approve_manager'):
                            ...
                            if employee.id != vals.get('approve_manager'):
                                employee.write({'manager_job_id': emp.job_id.id,'parent_id':rec.manager_id.id, 'approve_manager': vals.get('approve_manager')})
                            else:
                                job_line = job_line_obj.sudo().search(
                                    [('job_id', '=', res.job_id.id), ('company_id', '=', company_id),
                                     ('branch_id', '=', branch_id), ('department_id', '=', res.id)], limit=1)
